...
export function MetricsTable({ position }: MetricsTableProps) {
  const metrics = position.analytics.variables;
  ...
  const rows = [
    ...
    },
    {
      ...
      value: metrics.IL.value,
      valueColor: metrics.IL < 0 ? 'text-red-500' : 'text-solana-green',
    },
    {
      label: 'Total APR',
      value: formatPercentage(metrics.TotalAPR.value),
      valueColor: 'text-white',
    },
    {
      label: 'Fee APR',
      value: formatPercentage(metrics.FeeAPR.value),
      valueColor: 'text-white',
    }
    // {
